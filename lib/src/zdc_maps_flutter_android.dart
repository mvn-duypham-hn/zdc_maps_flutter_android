...
import 'zdc_map_inspector_android.dart';
...
class ZdcMapsFlutterAndroid extends ZdcMapsFlutterPlatform {
  ...
  MethodChannel ensureChannelInitialized(int mapId) {
    ...
    if (channel == null) {
      channel = MethodChannel('vn.duypx/zdc_maps_android_$mapId');
  ...
  /// Set [ZdcMapsFlutterPlatform] to use [AndroidViewSurface] to build the
  /// Zdc Maps widget.
  ///
  ...
  /// Requests Zdc Map Renderer with [AndroidMapRenderer] type.
  ///
  /// See https://pub.dev/packages/google_maps_flutter_android#map-renderer
  /// for more information.
  ///
  /// The renderer must be requested before creating ZdcMap instances as the
  ...
  Widget _buildView(
    ...
    const String viewType = 'vn.duypx/zdc_maps_android';
  ...
  void enableDebugInspection() {
    ZdcMapsInspectorPlatform.instance =
        ZdcMapsInspectorAndroid((int mapId) => _channel(mapId));
  }
}
